...
var gs={
  ...
  dizzycanvas:null,
  dizzyctx:null,
  x:154, // x position, from left
  y:55, // y position, from top
...
function drawroom(roomnum)
{
  ...
  var roomoffs=roomtable[roomnum].offs+2;
  ...
  {
    framenum=roomdata[roomoffs+(ptr++)];
    framex=roomdata[roomoffs+(ptr++)];
    framey=roomdata[roomoffs+(ptr++)];

    // Check for change of attributes
    if (framex<=0x7f)
    {
      frameattrib=roomdata[roomoffs+(ptr++)];
    ...
    // Check for flames
    if ((framenum==97) || (framenum==98) || (framenum==99))
      gs.flames.push({"frm":framenum,"x":framex,"y":framey,"col":frameattrib,"delay":gs.flamerate});

    // Check for water
    if ((framenum==100) || (framenum==101) || (framenum==102) || (framenum==103) || (framenum==104))
      gs.water.push({"frm":framenum,"x":framex,"y":framey,"col":(frameattrib & 0xe7),"delay":gs.waterrate});
...
/*
      ...
      {
        gs.ctx.save();
        gs.ctx.fillStyle="rgba(255,255,255,0.5)";
...
*/
  ...
  for (var i=0; i<objects.length; i++)
  {
    ...
    if (objects[i][0]==roomnum)
    {
      var objattrib=objects[i][3]
      var objreverse=((objattrib&0x80)!=0);
      var objplot=((objattrib&0x18)>>3);
      var objcolour=(objattrib&0x07);

      drawframe(gs.ctx, (objects[i][1]*4)-128, objects[i][2], objects[i][4], 1, objreverse, getpalette(objcolour), objplot, true);

      {
        var offs=frametable[objects[i][4]];
        var fx=(objects[i][1]*4)-128;
        var fy=objects[i][2];
        var fwidth=framedefs[offs++]*4;
        var fheight=framedefs[offs++];
        var framesolid=((objattrib&0x40)==0);
        //var framesolid=false; // Force objects non-solid for now
        ...
        setsolidity(fx, fy, fwidth, fheight, framesolid);
        setpackedsolidity(objects[i][1], objects[i][2], fwidth/4, fheight, framesolid);
...
*/
 ...
 writestring(gs.ctx, 6*8, 3*8, "ROOM:"+roomnum+"::", 1, getpalette(6), false);
...
{
  var imageData=gs.ctx.getImageData(x,y,1,1);
...
function collide(x, y)
{
  ...
  {
    // Also check underneath Dizzy (for slopes)
    var nonblack=0;

    for (var dx=0; dx<16; dx++)
      if (!isblack(x+dx, y+16)) nonblack++;
...
function update()
{
  ...
  for (var i=0; i<gs.water.length; i++)
  {
    ...
    gs.water[i].frm++;
    if (gs.water[i].frm>104) gs.water[i].frm=100;
...
{
  for (dframe=101; dframe<105; dframe++)
  {
